...
uses
  Winapi.Messages, Winapi.Windows, System.Classes, System.Contnrs, System.Generics.Collections, System.Math, System.SysUtils,
  System.UITypes, Vcl.Controls, Vcl.Dialogs, Vcl.ExtCtrls, Vcl.Forms, Vcl.Graphics, Vcl.StdCtrls, TextEditor.ActiveLine, TextEditor.Border,
  TextEditor.Caret, TextEditor.CodeFolding, TextEditor.CodeFolding.Hint.Form, TextEditor.CodeFolding.Ranges,
...
type
  ...
  strict private type
    ...
    TTextEditorScrollHelper = record
      ...
      HorizontalScrollMax: Integer;
      HorizontalShown: Boolean;
      HorizontalVisible: Boolean;
      IsScrolling: Boolean;
      LastHorizontalMax: Integer;
      LastHorizontalPosition: Integer;
      LastVerticalMax: Integer;
      LastVerticalPosition: Integer;
      PageWidth: Integer;
      Shadow: TTextEditorScrollShadowHelper;
      Timer: TTextEditorTimer;
      VerticalShown: Boolean;
...
{$ENDIF}
    ...
    procedure CMGestureManagerChanged(var AMessage: TMessage); message CM_GESTUREMANAGERCHANGED;
...
{$ENDIF}
    ...
    procedure SetZoomPercentage(const APercentage: Integer);
...
{$ENDIF}
  ...
  protected
    ...
    procedure PaintActiveLineBorder(const AClipRect: TRect);
    ...
    procedure PaintSimpleTextLines(const AClipRect: TRect; const AFirstLine, ALastLine: Integer; const AMinimap: Boolean);
...
uses
  Winapi.GDIPAPI, Winapi.GDIPOBJ, Winapi.Imm, Winapi.ShellAPI, System.Character, System.Generics.Defaults, System.RegularExpressions,
  System.Types, Vcl.Clipbrd, Vcl.ImgList, Vcl.Menus, Vcl.ToolWin, TextEditor.Export.HTML, TextEditor.Highlighter.Rules,
  TextEditor.Language, TextEditor.LeftMargin.Border, TextEditor.LeftMargin.LineNumbers, TextEditor.Search.Map, TextEditor.Search.Normal,
  TextEditor.Search.RegularExpressions, TextEditor.Search.WildCard, TextEditor.Undo.Item
{$IFDEF BASENCODING}
  , TextEditor.Encoding
{$ENDIF}
...
type
  ...
  TTextEditorHintWindow = class(THintWindow)
  strict private
    FBorderColor: TColor;
    FTextColor: TColor;
  protected
  ...
  public
    ...
    procedure SetColors(const ABackground, ABorder, AText: TColor);
...
function GetHintWindow(const AEditor: TCustomTextEditor): THintWindow;
var
  LHintWindowClass: THintWindowClass;
begin
  if eoNativeHintWindow in AEditor.Options then
    LHintWindowClass := HintWindowClass
  else
    LHintWindowClass := TTextEditorHintWindow;

  if Assigned(GHintWindow) and (GHintWindow.ClassType <> LHintWindowClass) then
...
constructor TTextEditorHintWindow.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);

  FBorderColor := TColors.SysWindowFrame;
  FTextColor := TColors.SysInfoText;

  Color := TColors.SysInfoBk;
end;

procedure TTextEditorHintWindow.SetColors(const ABackground, ABorder, AText: TColor);
begin
  Color := ABackground;
...
function TTextEditorHintWindow.CalcHintRect(MaxWidth: Integer; const AHint: string; AData: TCustomData): TRect;
begin
  Result := System.Types.Rect(0, 0, MaxWidth, 0);

  Canvas.Font.Assign(Font);

  Winapi.Windows.DrawText(Canvas.Handle, AHint, -1, Result, DT_CALCRECT or DT_LEFT or DT_WORDBREAK or DT_NOPREFIX or
...
procedure TTextEditorHintWindow.NCPaint(DC: HDC);
var
  LBrush: HBRUSH;
  LRect: TRect;
begin
  LRect := System.Types.Rect(0, 0, Width, Height);
  LBrush := CreateSolidBrush(ColorToRGB(FBorderColor));
  try
    FrameRect(DC, LRect, LBrush);
  finally
    DeleteObject(LBrush);
  end;
end;

procedure TTextEditorHintWindow.Paint;
var
  LRect: TRect;
begin
  LRect := ClientRect;
  ...
  Winapi.Windows.DrawText(Canvas.Handle, Caption, -1, LRect, DT_LEFT or DT_NOPREFIX or DT_WORDBREAK or DrawTextBiDiModeFlagsReadingOnly);
end;
...
{$ENDIF}
  ...
  FMouse.ScrollCursors[TMouseWheelScrollCursors.North] := LoadCursor(0, IDC_SIZENS);
  FMouse.ScrollCursors[TMouseWheelScrollCursors.NorthEast] := LoadCursor(0, IDC_SIZENESW);
  FMouse.ScrollCursors[TMouseWheelScrollCursors.East] := LoadCursor(0, IDC_SIZEWE);
  FMouse.ScrollCursors[TMouseWheelScrollCursors.SouthEast] := LoadCursor(0, IDC_SIZENWSE);
  FMouse.ScrollCursors[TMouseWheelScrollCursors.South] := LoadCursor(0, IDC_SIZENS);
  FMouse.ScrollCursors[TMouseWheelScrollCursors.SouthWest] := LoadCursor(0, IDC_SIZENESW);
  FMouse.ScrollCursors[TMouseWheelScrollCursors.West] := LoadCursor(0, IDC_SIZEWE);
  FMouse.ScrollCursors[TMouseWheelScrollCursors.NorthWest] := LoadCursor(0, IDC_SIZENWSE);
...
procedure TCustomTextEditor.PaintActiveLineBorder(const AClipRect: TRect);
var
  LColor: TColor;
  LLine, LFirstRow, LLastRow, LLastVisibleRow, LLineHeight, LTop, LRight, LThickness, LIndex: Integer;
  LRect: TRect;
begin
  ...
  LRect := Rect(FLeftMarginWidth, LTop, LRight, LTop + (LLastRow - LFirstRow + 1) * LLineHeight);
...
var
  ...
  LRect: TRect;
begin
  if (FBorder.Style = bsSingle) and (FBorder.ColoredEdges <> []) and not IsCustomStyleActive then
  begin
    ...
    try
      ...
      LOldDC := Canvas.Handle;
      Canvas.Handle := LDC;
      Canvas.Pen.Color := FBorder.Color;

      if ebLeft in FBorder.ColoredEdges then
        Canvas.Polyline([Point(LRect.Left, LRect.Bottom - 1), Point(LRect.Left, LRect.Top - 1)]);

      if ebTop in FBorder.ColoredEdges then
        Canvas.Polyline([Point(LRect.Left, LRect.Top), Point(LRect.Right, LRect.Top)]);

      if ebRight in FBorder.ColoredEdges then
        Canvas.Polyline([Point(LRect.Right - 1, LRect.Top), Point(LRect.Right - 1, LRect.Bottom)]);

      if ebBottom in FBorder.ColoredEdges then
        Canvas.Polyline([Point(LRect.Right - 1, LRect.Bottom - 1), Point(LRect.Left - 1, LRect.Bottom - 1)]);

      Canvas.Handle := LOldDC;
    finally
...
begin
  Result := Max(ClientRect.Right - FLeftMargin.GetWidth - GetCodeFoldingWidth - 2 - FMinimap.GetWidth - FSearch.Map.GetWidth, 0);
end;
...
function TCustomTextEditor.GetTokenWidth(const AToken: string; const ALength: Integer; const ACharsBefore: Integer; const AMinimap: Boolean = False; const ARTLReading: Boolean = False): Integer;
...
var
  ...
  var
    LMaxWidth: Integer;
    LTextLine: string;
    LWidth, LLength, LViewLength, LCharsBefore: Integer;
    LTokenLength, LTokenWidth: Integer;
...
function TCustomTextEditor.ViewPositionToPixels(const AViewPosition: TTextEditorViewPosition; const ALineText: string = ''): TPoint;
var
  LRow: Integer;
  LPositionY: Integer;
  LLineText: string;
...
begin
  ...
  for var LRetryCount := 1 to TClipboardDefaults.MaxRetries do
  try
    Clipboard.Open;
    Exit(True);
  except
...
var
  LRow, LCurrentRow, LXInEditor: Integer;
  LLineText: string;
  LFontStyles, LPreviousFontStyles: TFontStyles;
  LTextWidth, LCharsBefore, LLength: Integer;
  LHighlighterAttribute: TTextEditorHighlighterAttribute;
  LToken, LNextTokenText: string;
  LTokenWidth, LTokenLength: Integer;
  LPToken: PChar;
...
begin
  ...
  LRow := ARow;
  LXInEditor := X + FScrollHelper.HorizontalPosition - FLeftMarginWidth + FPaintHelper.CharWidth shr 1 + 1;

  if FEditorMode = emSimple then
  begin
    Result.Column := LXInEditor div FPaintHelper.CharWidth + 1;
    Result.Row := ARow;
    Exit;
  end;
...
procedure TCustomTextEditor.BookmarkListChanged(ASender: TObject); //FI:O804 Method parameter is declared but never used
begin
  Invalidate;
end;

procedure TCustomTextEditor.BorderStyleChanged(ASender: TObject); //FI:O804 Method parameter is declared but never used
begin
  RecreateWnd;
end;

procedure TCustomTextEditor.BorderChanged(ASender: TObject); //FI:O804 Method parameter is declared but never used
begin
  BorderWidth := FBorder.Width;
...
begin
  ...
  begin
    ...
    if APoint.X < LScrollBounds.Left then
      ...
      FScrollHelper.Delta.X := if APoint.X >= LScrollBounds.Right then (APoint.X - LScrollBounds.Right) div FPaintHelper.CharWidth + 1 else 0;

    if APoint.Y < LScrollBounds.Top then
      FScrollHelper.Delta.Y := (APoint.Y - LScrollBounds.Top) div GetLineHeight - 1
    else
      FScrollHelper.Delta.Y := if APoint.Y >= LScrollBounds.Bottom then (APoint.Y - LScrollBounds.Bottom) div GetLineHeight + 1 else 0;
...
begin
  FLines[ALine] := if eoTrimTrailingSpaces in Options then TextEditor.Utils.TrimRight(ALineText) else ALineText;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      LSpaceCount := LLineText.Length - TextEditor.Utils.TrimLeft(LLineText).Length;
...
begin
  ...
  FUndoList.BeginBlock;
  try
    FUndoList.AddChange(crCaret, TextPosition, SelectionStartPosition, SelectionStartPosition, '',
...
var
  LPoint: TPoint;
  X, Y: Integer;
  LCaretHeight, LCaretWidth: Integer;
...
var
  ...
  LRGBColor: Cardinal;
  LLineHeight, LSize: Integer;
  LX, LBottom: Single;
begin
  LLineHeight := GetLineHeight;
  LSize := Max(4, LLineHeight div 3);
  LRGBColor := ColorToRGB(FColors.BookmarkRed);
  ...
  try
    ...
    begin
      ...
      LPoints[0] := MakePoint(LX, LBottom - LSize);
      LPoints[1] := MakePoint(LX + LSize * 0.75, LBottom);
      LPoints[2] := MakePoint(LX - LSize * 0.75, LBottom);
...
procedure TCustomTextEditor.CMGestureManagerChanged(var AMessage: TMessage);
begin
  if not (csDestroying in ComponentState) then
  begin
    ControlStyle := if Assigned(Touch.GestureManager) then ControlStyle + [csGestures] else ControlStyle - [csGestures];
...
procedure TCustomTextEditor.CMMouseLeave(var AMessage: TMessage);
begin
  inherited;
...
procedure TCustomTextEditor.UpdateCodeFoldingGutterHover(const AX: Integer);
var
  LMouseOver: Boolean;
begin
...
procedure TCustomTextEditor.MatchingPairsChanged(ASender: TObject); //FI:O804 Method parameter is declared but never used
begin
...
begin
  ...
  try
    ...
    if FScrollHelper.Delta.Y <> 0 then
      TopLine := if GetKeyState(vkShift) < 0 then TopLine + FScrollHelper.Delta.Y * FLineNumbers.VisibleCount else TopLine + FScrollHelper.Delta.Y;
...
begin
  ...
  begin
    BlendFunction.SourceConstantAlpha := FScroll.Shadow.AlphaBlending;
    ...
    Bitmap.Canvas.Brush.Color := FScroll.Shadow.Color;
...
begin
  ...
  try
    ...
    if FScrollHelper.Delta.Y <> 0 then
    begin
      TopLine := TopLine + if GetKeyState(vkShift) < 0 then FScrollHelper.Delta.Y * FLineNumbers.VisibleCount else FScrollHelper.Delta.Y;
...
var
  ...
  begin
    ...
    begin
      ...
      while (LPrefixIndex > LIndex) and (LBlockEndPosition.Char <= LLength) and
        (LTempString[LBlockEndPosition.Char] = LTempString[LPrefixIndex]) do
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if FScrollHelper.HorizontalShown then
      begin
        ShowScrollBar(Handle, SB_HORZ, False);
        FScrollHelper.HorizontalShown := False;
      end;
    end
    else
    begin
      ...
      if FScrollHelper.HorizontalScrollMax <= TMaxValues.ScrollRange then
      begin
        LScrollInfo.nMax := FScrollHelper.HorizontalScrollMax;
        LScrollInfo.nPage := FScrollHelper.PageWidth;
        LScrollInfo.nPos := Min(FScrollHelper.HorizontalPosition, FScrollHelper.HorizontalScrollMax);
      end
      else
      begin
        LScrollInfo.nMax := TMaxValues.ScrollRange;
        LScrollInfo.nPage := MulDiv(TMaxValues.ScrollRange, FScrollHelper.PageWidth, FScrollHelper.HorizontalScrollMax);
        LScrollInfo.nPos := MulDiv(TMaxValues.ScrollRange, FScrollHelper.HorizontalPosition, FScrollHelper.HorizontalScrollMax);
      end;

      if (FScrollHelper.LastHorizontalPosition <> LScrollInfo.nPos) or (FScrollHelper.LastHorizontalMax <> LScrollInfo.nMax) then
      begin
        FScrollHelper.LastHorizontalPosition := LScrollInfo.nPos;
        FScrollHelper.LastHorizontalMax := LScrollInfo.nMax;
        ...
        LVisible := FScrollHelper.HorizontalVisible and (FScroll.Bars in [ssBoth, ssHorizontal]);

        if LVisible <> FScrollHelper.HorizontalShown then
        begin
          ShowScrollBar(Handle, SB_HORZ, LVisible);
          FScrollHelper.HorizontalShown := LVisible;
        end;
        ...
        begin
          ...
          if soAutoDisableScrollButtons in FScroll.Options then
          begin
            if LScrollInfo.nPos = 0 then
              EnableScrollBar(Handle, SB_HORZ, ESB_DISABLE_LEFT)
            else
            if LScrollInfo.nPos + Integer(LScrollInfo.nPage) >= LScrollInfo.nMax then
    ...
    if (FScrollHelper.LastVerticalPosition <> LScrollInfo.nPos) or (FScrollHelper.LastVerticalMax <> LScrollInfo.nMax)then
    begin
      FScrollHelper.LastVerticalPosition := LScrollInfo.nPos;
      FScrollHelper.LastVerticalMax := LScrollInfo.nMax;
      ...
      LVisible := FScrollHelper.VerticalVisible and (FScroll.Bars in [ssBoth, ssVertical]);

      if LVisible <> FScrollHelper.VerticalShown then
      begin
        ShowScrollBar(Handle, SB_VERT, LVisible);
        FScrollHelper.VerticalShown := LVisible;
      end;
  ...
  if FScrollHelper.HorizontalShown or FScrollHelper.VerticalShown then
  begin
    ShowScrollBar(Handle, SB_BOTH, False);
    FScrollHelper.HorizontalShown := False;
    FScrollHelper.VerticalShown := False;

    { Force a full reapply when content returns }
    FScrollHelper.LastHorizontalMax := -1;
    FScrollHelper.LastHorizontalPosition := -1;
    FScrollHelper.LastVerticalMax := -1;
    FScrollHelper.LastVerticalPosition := -1;
  end;
end;
  ...
  try
    ...
    begin
      ...
      try
        ...
        for var LIndex := 0 to LNumberDropped - 1 do
        begin
...
begin
  AMessage.Result := if (csDocking in ControlState) or (csDestroying in ComponentState) then 0 else FLines.GetTextLength;
end;
...
procedure TCustomTextEditor.WMNCCalcSize(var AMessage: TWMNCCalcSize);
begin
  inherited;

  if (FBorder.ColoredEdges = []) or IsCustomStyleActive then
    Exit;

  with AMessage.CalcSize_Params^ do
  begin
    if ebLeft in FBorder.ColoredEdges then
      Dec(rgrc[0].Left);
...
procedure TCustomTextEditor.WMNCPaint(var AMessage: TWMNCPaint);
begin
  inherited;
...
begin
  AMessage.Result := 1;
  try
    Text := if HandleAllocated and IsWindowUnicode(Handle) then PChar(AMessage.Text) else string(PAnsiChar(AMessage.Text));
  except
    AMessage.Result := 0;
        ...
        try
          ...
          TopLine :=
            if LVerticalMaxScroll <= TMaxValues.ScrollRange then
              ...
              MulDiv(LVerticalMaxScroll, Min(Abs(AMessage.Pos), TMaxValues.ScrollRange), TMaxValues.ScrollRange);

          if soShowVerticalScrollHint in FScroll.Options then
          begin
            LScrollHintWindow := GetHintWindow(Self);
            LScrollHint :=
...
begin
  Result := MessageDialog(Format(STextEditorSearchMatchNotFound, [sDoubleLineBreak]), mtConfirmation, [mbYes, mbNo], mbYes) = mrYes;
end;
...
begin
  ...
  with AParams do
  begin
    Style := Style or BorderStyles[FBorder.Style];

    if NewStyleControls and Ctl3D and (FBorder.Style = bsSingle) then
    begin
...
procedure TCustomTextEditor.DoGetGestureOptions(var Gestures: TInteractiveGestures; var Options: TInteractiveGestureOptions);
begin
...
procedure TCustomTextEditor.DoOnLeftMarginClick(AButton: TMouseButton; AShift: TShiftState; X, Y: Integer);
  ...
  begin
    ...
    LBookmarkColors := [STextEditorBookmarkYellow, STextEditorBookmarkRed, STextEditorBookmarkGreen, STextEditorBookmarkBlue, STextEditorBookmarkPurple];
...
{$IFDEF ALPHASKINS}
    if Assigned(SkinData.SkinManager) and SkinData.SkinManager.IsActive then
      SkinData.SkinManager.SkinableMenus.HookPopupMenu(LPopupMenu, True);
{$ENDIF}
...
begin
  ...
  for var LLine := AFromLine to AToLine do
  begin
    SetHighlighterRange(LLine);
    ...
    begin
      ...
      if LWord.Length > 1 then
        if LCommentsFound and (FHighlighter.TokenType in [ttBlockComment, ttLineComment, ttString]) or not LCommentsFound then
        begin
          LIsWord := True;
          LPWord := PChar(LWord);
...
begin
  ...
  try
    ...
    begin
      ...
      TextPosition := GetPosition(LTextPosition.Char + LWord.Length, LTextPosition.Line);
    end;
  finally
    ClearMinimapBuffer;
...
begin
  ...
  if (FEditorMode <> emSimple) and not ReadOnly and FSyncEdit.Active or (X + 4 > FLeftMarginWidth) and ((AButton = mbLeft) or (AButton = mbRight)) then
    LTextPosition := PixelsToTextPosition(X, Y);
  ...
  if FRightMargin.Visible and (rmoMouseMove in FRightMargin.Options) then
    if (AButton = mbLeft) and (Abs(FRightMargin.Position * FPaintHelper.CharWidth + FLeftMarginWidth - X - FScrollHelper.HorizontalPosition) < 3) then
    begin
  ...
  if IsCodeFoldingVisible and (AButton = mbLeft) and FCodeFolding.Hint.Indicator.Visible and (cfoExpandByHintClick in FCodeFolding.Options) and (FLines.Count > 0) then
...
begin
  ...
  LHintWindow := GetHintWindow(Self);
...
begin
  LHintWindow := GetHintWindow(Self);
  LPositionText := Format(STextEditorRightMarginPosition, [(FRightMarginMovePosition - FLeftMarginWidth + FScrollHelper.HorizontalPosition) div FPaintHelper.CharWidth]);
  LRect := LHintWindow.CalcHintRect(200, LPositionText, nil);
...
begin
  ...
  try
    ...
    begin
      ...
      PaintSimpleTextLines(LDrawRect, LLine1, LLine2, False);
    end
    else
    ...
    { Minimap shadow }
    if FMinimap.Visible and FMinimap.Shadow.Visible then
    begin
      LDrawRect := ClientRect;

      LDrawRect.Left := FLeftMarginWidth - FLeftMargin.GetWidth - GetCodeFoldingWidth;
...
begin
  ...
  begin
    ...
    if FActiveLine.Visible and (aloHighlightLeftMargin in FActiveLine.Options) and
      (FColors.CodeFoldingActiveLineBackground <> TColors.SysNone) and
    ...
    begin
      Canvas.Brush.Color := if Focused then FColors.CodeFoldingActiveLineBackground else FColors.CodeFoldingActiveLineBackgroundUnfocused;
  ...
  begin
    ...
    begin
      case CodeFolding.MarkStyle of
        msSquare:
          begin
            { FrameRect is always one pixel - draw inward rings so the border scales with zoom without
              bleeding outside the rect }
            LTempRect := LRect;

            for var LIndex := 1 to LThickness do
            begin
              Canvas.FrameRect(LTempRect);
              InflateRect(LTempRect, -1, -1);
            end;
          end;
        msCircle:
          begin
      ...
      { The - and + are filled rectangles - thick GDI lines center on the path with the spare pixel of an
        even width going up and left, and round end caps overshoot the endpoints }

      { - }
      LTempY := LRect.Top + (LRect.Bottom - LRect.Top - LThickness) shr 1;

      Canvas.FillRect(Rect(LRect.Left + LRect.Width div 4, LTempY, LRect.Right - LRect.Width div 4,
        LTempY + LThickness));

      if LFoldRange.Collapsed then
      begin
        { + }
        LTempX := LRect.Left + (LRect.Right - LRect.Left - LThickness) shr 1;

        Canvas.FillRect(Rect(LTempX, LRect.Top + LRect.Height div 4, LTempX + LThickness,
          LRect.Bottom - LRect.Height div 4));
      end;
    end;
...
begin
  ...
  { AutoHide: only marks of collapsed ranges stay visible until the mouse is over the gutter }
  if FCodeFolding.AutoHide and not FCodeFoldings.MouseOverGutter and
  ...
  InflateRect(LRect, MulDiv(-2, FPixelsPerInch, 96), 0);
...
var
  ...
  LCollapseMarkRect: TRect;
  LBrush: TBrush;
  LIndex, LDotSize, LBoxHeight, LX, LY: Integer;
  LPoints: array [0..2] of TPoint;
begin
  LOldPenColor := Canvas.Pen.Color;
  LOldBrushColor := Canvas.Brush.Color;
  ...
  begin
    ...
    { Compact content-derived box: three zoom-scaled dots with dot-sized gaps and margins, about 0.7 of the
      line height, vertically centered }
    LDotSize := Max(2, MulDiv(2, FPixelsPerInch, 96));
    LBoxHeight := MulDiv(ALineRect.Height, 7, 10);
    ...
    LCollapseMarkRect.Top := ALineRect.Top + (ALineRect.Height - LBoxHeight) shr 1;
        ...
        begin
          ...
          case FCodeFolding.Hint.Indicator.MarkStyle of
            imsThreeDots:
              begin
                { [...] }
                LY := LCollapseMarkRect.Top + (LCollapseMarkRect.Height - LDotSize) shr 1;
                LX := LCollapseMarkRect.Left + (LCollapseMarkRect.Width - 5 * LDotSize) shr 1;

                for LIndex := 1 to 3 do
                begin
                  Canvas.Rectangle(LX, LY, LX + LDotSize, LY + LDotSize);
                  Inc(LX, 2 * LDotSize);
                end;
              end;
            imsTriangle:
...
var
  LOldColor: TColor;
  LLineHeight, LDeepestLevel, LY: Integer;
  LHideAtFirstColumn, LHideOverText, LHideInActiveRow, LHighlightIndentGuides, LSkip: Boolean;
...
var
  ...
  begin
    if Assigned(FLeftMargin.Marks.Images) then
      ...
      begin
        LY := if LLineHeight > FLeftMargin.Marks.Images.Height then LLineHeight shr 1 - FLeftMargin.Marks.Images.Height shr 1 else 0;
  ...
  var
    ...
    LLeftMarginWidth, LLongLineWidth: Integer;
    LLineNumber: string;
    LTop: Integer;
    LBackground: TColor;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            if not Assigned(LGPGraphics) then
            begin
              ...
              LColorRGB := ColorToRGB(FColors.LeftMarginLineNumberLine);
              LGPBrush := TGPSolidBrush.Create(MakeColor(255, GetRValue(LColorRGB), GetGValue(LColorRGB), GetBValue(LColorRGB)));
            end;

            LTop := LLineRect.Top + (LLineHeight shr 1);
            LCenterX := LLeftMarginWidth - LMargin - 1.5 - (FLeftMarginCharWidth / 2);

            if LLine mod 5 = 0 then
            begin
              LSize := Round(Max(2, FLeftMarginCharWidth / 4)) - 1;

              LGPGraphics.FillRectangle(LGPBrush, Trunc(LCenterX - (LLongLineWidth / 2) + 0.5), LTop - LSize shr 1, LLongLineWidth, LSize);
            end
            else
            begin
              LSize := Max(2, Round(FLeftMarginCharWidth * 0.3));

              if LSize < 4 then
                LGPGraphics.FillRectangle(LGPBrush, Trunc(LCenterX - (LSize / 2) + 0.5), LTop - LSize shr 1, LSize, LSize)
              else
                LGPGraphics.FillEllipse(LGPBrush, Trunc(LCenterX - (LSize / 2) + 0.5), LTop - LSize shr 1, LSize, LSize);
            end;
  ...
  var
    ...
    begin
      ...
      LPanelActiveLineRect := System.Types.Rect(AClipRect.Left, LTop, AClipRect.Left + FLeftMargin.MarksPanel.Width, LTop + LLineHeight);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if FActiveLine.Visible and (aloHighlightLeftMargin in FActiveLine.Options) and
          ((FColors.LeftMarginActiveLineBackground <> TColors.SysNone) and
        ...
        begin
          ...
          Canvas.Brush.Color := if Focused then FColors.LeftMarginActiveLineBackground else FColors.LeftMarginActiveLineBackgroundUnfocused;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Canvas.Brush.Color := if LLineState = lsNormal then FColors.LeftMarginLineStateNormal else FColors.LeftMarginLineStateModified;
...
begin
  ...
  LLeft := if FMinimap.Align = maLeft then AClipRect.Left else AClipRect.Right - FMinimapHelper.Shadow.Bitmap.Width;
  ...
  var
    ...
    begin
      if LRunStartColumn >= 0 then
        Canvas.FillRect(Rect(LLeft + LRunStartColumn * LCharWidth, ATop,
          Min(LLeft + LColumn * LCharWidth, AClipRect.Right - 2), LBottom));
...
begin
  ...
  if (moShowSearchResults in FMinimap.Options) and (FSearch.Items.Count > 0) and
    (FColors.SearchHighlighterBackground <> TColors.SysNone) then
  begin
    ...
    begin
      ...
      begin
        ...
        Canvas.FillRect(Rect(LLeft + (LSearchItem.BeginTextPosition.Char - 1) * LCharWidth, LTop,
          Min(LLeft + (LSearchItem.EndTextPosition.Char - 1) * LCharWidth, AClipRect.Right - 2),
          LTop + Max(LCharHeight - 1, 1)));
      end;
    end;
  end;
end;
...
begin
  ...
  { Background }
  Canvas.Brush.Color := if FColors.SearchMapBackground <> TColors.SysNone then FColors.SearchMapBackground else FColors.EditorBackground;

  FillRect(LRect);

  { Lines in window }
  LHeight := ClientHeight div Max(FLines.Count, 1);
        ...
        case FSpecialChars.LineBreak.Style of
          eolCRLF:
            begin
              ...
              begin
                FPaintHelper.SetBackgroundColor(Colors.SelectionBackground);
                FPaintHelper.SetForegroundColor(Colors.SelectionForeground);
              end;
          ...
          eolPilcrow:
            begin
              ...
              Winapi.Windows.ExtTextOut(Canvas.Handle, LCharRect.Left, LCharRect.Top + FLineSpacing shr 1, ETO_OPTIONS,
...
{$IFDEF TEXT_EDITOR_SPELL_CHECK}
function TCustomTextEditor.GetFirstSpellCheckIndex(const AMinimap: Boolean): Integer;
var
  LTextPosition: TTextEditorTextPosition;
begin
  Result := -1;

  if not AMinimap and Assigned(FSpellCheck) and Assigned(FSpellCheck.Items) and (FSpellCheck.Items.Count > 0) then
  begin
    Result := 0;

    while Result < FSpellCheck.Items.Count do
    begin
      LTextPosition := PTextEditorTextPosition(FSpellCheck.Items.Items[Result])^;
    ...
    if Result = FSpellCheck.Items.Count then
      Result := -1;
  end;
end;
{$ENDIF}
...
var
  ...
  begin
    ...
    if LIsCurrentLine and FActiveLine.Visible and (FActiveLine.Style = alsFill) and not Focused and
      (FColors.ActiveLineBackgroundUnfocused <> TColors.SysNone) then
...
var
  ...
  begin
    ...
    if ASelected then
    begin
      FPaintHelper.SetForegroundColor(if FColors.SelectionForeground <> TColors.SysNone then FColors.SelectionForeground else LForegroundColor);
    ...
    begin
      ...
      begin
        ...
        if not LToken.IsEmpty then
          Winapi.Windows.ExtTextOut(Canvas.Handle, LSearchRect.Left, LSearchRect.Top + (if AMinimap then 0 else FLineSpacing shr 1),
            ETO_OPTIONS, @LSearchRect, PChar(LToken), LToken.Length, nil);
...
var
  ...
  LTokenRect, LSelectedRect: TRect;
  ...
  var
    ...
    LLastChar: Char;
    LAnsiChar: AnsiChar;
    LBottom, LMaxX, LOrigMaxX: Integer;
    LLeftStart: Byte;
    LPixels: PRGBTripleArray;
    LTriple: TRGBTriple;
    LOldPenColor: TColor;
    LStep, LLeft, LTop: Integer;
  begin
    ...
    begin
      ...
      begin
        ...
        if (FSpecialChars.Selection.Visible and (Canvas.Brush.Color = FColors.SelectionBackground) or (Canvas.Brush.Color <> FColors.SelectionBackground)) then
        begin
      ...
      begin
        ...
        begin
          ...
          try
            ...
            Winapi.Windows.ExtTextOut(LTempBitmap.Canvas.Handle, LTextRect.Left, (if AMinimap then 0 else FLineSpacing shr 1),
              ETO_OPTIONS, @LTempRect, LPChar, LTokenLength, nil);

            BitBlt(Canvas.Handle, LSelectedRect.Left, LSelectedRect.Top, LSelectedRect.Width, LSelectedRect.Height, LTempBitmap.Canvas.Handle, LSelectedRect.Left, 0, SRCCOPY);
          finally
            LTempBitmap.Free;
          end;
        end
        else
          Winapi.Windows.ExtTextOut(Canvas.Handle, LTextRect.Left, LTextRect.Top + (if AMinimap then 0 else FLineSpacing shr 1),
            ETO_OPTIONS, @LTextRect, LPChar, LTokenLength, nil);
        ...
        begin
          ...
          begin
            ...
            BitBlt(FItalic.Bitmap.Canvas.Handle, 0, 0, FItalic.Bitmap.Width, FItalic.Bitmap.Height, Canvas.Handle, LMaxX, LTokenRect.Top, SRCCOPY);
            ...
            begin
              ...
              begin
                ...
                if (LTriple.Red <> LBackgroundColorRed) and (LTriple.Green <> LBackgroundColorGreen) and (LTriple.Blue <> LBackgroundColorBlue) and (LOrigMaxX + LLeft > LMaxX) then
                begin
    ...
    begin
      ...
      begin
        ...
        LCanAppend := LCanAppend and
          ((LTokenHelper.FontStyle = AFontStyle) or ((LEmptySpace <> TTextEditorEmptySpace.esNone) and not (fsUnderline in AFontStyle) and
...
var
  ...
  LTokenText: string;
    ...
    var
      LBorderColor, LUnderlineColor: TColor;
    ...
    begin
      ...
      begin
        LForegroundColor := if LHighlighterAttribute.Foreground = TColors.SysNone then Colors.EditorForeground else LHighlighterAttribute.Foreground;
        ...
        if (LMarkColor <> TColors.SysNone) and not (LIsCurrentLine and FActiveLine.Visible and
          (FActiveLine.Style = alsFill) and (FColors.ActiveLineBackground <> TColors.SysNone)) then
        begin
...
var
  ...
  begin
    ...
    begin
      ...
      LMarkColor := if AMinimap then TColors.SysNone else GetMarkBackgroundColor(LCurrentLine + 1);
      ...
      begin
        ...
        LLineRect.Top := LLineRect.Bottom;
        Inc(LLineRect.Bottom, if AMinimap then FMinimap.CharHeight else LLineHeight);
        Inc(LViewLine);
...
{$IFDEF TEXT_EDITOR_SPELL_CHECK}
  LCurrentSpellCheckIndex := GetFirstSpellCheckIndex(AMinimap);

  if LCurrentSpellCheckIndex <> -1 then
    LSpellCheckTextPosition := PTextEditorTextPosition(FSpellCheck.Items.Items[LCurrentSpellCheckIndex])^;
{$ENDIF}
...
procedure TCustomTextEditor.PaintSimpleTextLines(const AClipRect: TRect; const AFirstLine, ALastLine: Integer; const AMinimap: Boolean);
var
  LLineRect: TRect;
  ...
  begin
    ...
    begin
      if Focused then
      begin
        if FColors.ActiveLineBackground <> TColors.SysNone then
          LColor := FColors.ActiveLineBackground
        else
          Exit;
      end
      else
      if FColors.ActiveLineBackgroundUnfocused <> TColors.SysNone then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Winapi.Windows.ExtTextOut(Canvas.Handle, LTextRect.Left, LTextRect.Top + (if AMinimap then 0 else FLineSpacing shr 1),
          ETO_CLIPPED, @LTextRect, PChar(LUnselectedTextBefore), LUnselectedTextBefore.Length, nil);
      ...
      begin
        ...
        Winapi.Windows.ExtTextOut(Canvas.Handle, LTextRect.Left, LTextRect.Top + (if AMinimap then 0 else FLineSpacing shr 1),
          ETO_OPAQUE or ETO_CLIPPED, @LTextRect, PChar(LSelectedText), LSelectedText.Length, nil);
      ...
      begin
        Canvas.Brush.Color := FColors.ActiveLineBackground;
        FPaintHelper.SetForegroundColor(FColors.EditorForeground);

        Winapi.Windows.ExtTextOut(Canvas.Handle, LTextRect.Left, LTextRect.Top + (if AMinimap then 0 else FLineSpacing shr 1),
          ETO_OPAQUE or ETO_CLIPPED, @LTextRect, PChar(LUnselectedTextAfter), LUnselectedTextAfter.Length, nil);
      end;
    end
    else
    begin
      ...
      Winapi.Windows.ExtTextOut(Canvas.Handle, LLineRect.Left, LLineRect.Top + (if AMinimap then 0 else FLineSpacing shr 1),
        ETO_CLIPPED, @LLineRect, PChar(LText), LText.Length, nil);
    end;
  end;
...
begin
  ...
  begin
    ...
    SetCursor(if LCursorIndex = -1 then 0 else FMouse.ScrollCursors[LCursorIndex])
  end
  else
...
begin
  ...
  Dec(LTextPosition.Char, LWord.Length);
...
begin
  ...
  if (ATextPosition.Line = FLines.Count - 1) and (ATextPosition.Char > FLines.TextLines[FLines.Count - 1].Length) then
    Exit;
...
begin
  ...
  try
    ...
    while LFound do
    begin
      LFound := if LReplaceTextParams.Backwards then FindPrevious(False) else FindNext(False);
...
begin
  ...
  try
    ...
    begin
      LPText := PChar(FLines.TextLines[Line]);
...
begin
  ...
  begin
    ...
    Inc(LRect.Left, FLeftMargin.GetWidth + GetCodeFoldingWidth);
    ...
    if (FLast.ViewPosition.Row = LViewPosition.Row) and (FLast.ViewPosition.Column = LViewPosition.Column) then
      Exit;
    ...
    case AMessage.Msg of
      CM_SHOWINGCHANGED:
        begin
          ...
          if Assigned(FScrollHelper.Wnd) then
            SetWindowPos(Handle, 0, 0, 0, 0, 0, SWP_FRAMECHANGED or SWP_NOMOVE or SWP_NOSIZE or SWP_NOZORDER or SWP_NOACTIVATE);
        end;
...
procedure TCustomTextEditor.SetZoomPercentage(const APercentage: Integer);
var
