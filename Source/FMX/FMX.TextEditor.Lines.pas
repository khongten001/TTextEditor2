...
uses
  System.Classes, System.SysUtils, FMX.TextEditor.Consts, FMX.TextEditor.Types, FMX.TextEditor.Utils;
...
type
  ...
  strict private
    ...
    function GetEncoding(const AStream: TStream; const AEncoding: System.SysUtils.TEncoding): System.SysUtils.TEncoding;
    ...
    procedure Grow;
    procedure InvalidateTextCache; inline;
    procedure LoadFirstRows(const AStream: TStream; const AStringList: TStringList; const ARows: Integer; const AEncoding: System.SysUtils.TEncoding);
    procedure LoadLastRows(const AStream: TStream; const AStringList: TStringList; const ARows: Integer; const AEncoding: System.SysUtils.TEncoding);
  ...
  public
    ...
    function ContainsFlag(const AFlag: TTextEditorStringFlag): Boolean;
    ...
    procedure LoadPartFromStream(const AStream: TStream; const AFrom: TTextEditorPartialLoadFrom; const ARows: Integer; const AEncoding: System.SysUtils.TEncoding = nil);
    ...
    procedure SetLineBreakFlags(const ALineBreak: TTextEditorLineBreak);
...
uses
  System.Character, System.Math, FMX.TextEditor.Encoding, FMX.TextEditor.Language;

{ TTextEditorLines }

{$IFDEF TEXT_EDITOR_FMX_RANGE_CHECKS}
...
begin
  ...
  Result := if (FIndexOfLongestLine >= 0) and (FIndexOfLongestLine < FCount) then FItems^[FIndexOfLongestLine].ExpandedLength else 0;
end;
...
begin
  Result := if (AIndex < 0) or (AIndex > FCount - 1) then 0 else FItems^[AIndex].TextLine.Length;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Include(Flags, if LHasTabs then sfHasTabs else sfHasNoTabs);
...
begin
  ...
  if (LCount = 1) and (GetTextLines(0) = '') then
    Result := if LQuoteChar = #0 then '' else LQuoteChar + LQuoteChar
  else
  begin
    Result := '';
...
begin
  if IsValidIndex(AIndex) then
    Result := if sfHasNoTabs in FItems^[AIndex].Flags then GetStrings(AIndex) else ExpandString(AIndex)
  else
    Result := '';
end;
...
begin
  if IsValidIndex(AIndex) then
    Result := if sfExpandedLengthUnknown in FItems^[AIndex].Flags then ExpandedStrings[AIndex].Length else FItems^[AIndex].ExpandedLength
  else
    Result := 0;
end;
...
begin
  Result := if IsValidIndex(AIndex) then FItems^[AIndex].Range else nil;
end;
...
begin
  ...
  begin
    if FSavingToStream and TrimTrailingSpaces then
      TextLine := FMX.TextEditor.Utils.TrimRight(TextLine);
...
begin
  ...
  for var LIndex := AStart to AEnd - 1 do
  with FItems^[LIndex] do
  begin
    if FSavingToStream and TrimTrailingSpaces then
      TextLine := FMX.TextEditor.Utils.TrimRight(TextLine);
...
function TTextEditorLines.ContainsFlag(const AFlag: TTextEditorStringFlag): Boolean;
begin
  Result := False;

  for var LIndex := 0 to FCount - 1 do
  if AFlag in FItems^[LIndex].Flags then
    Exit(True);
end;
...
begin
  ...
  for var LIndex := Count - 1 downto 0 do
...
var
  LSize: Integer;
  LPValue: PChar;
  LIndex2, LLength, LLineBreakLength: Integer;
  LLineBreak: string;
  LStringRecord: TTextEditorStringRecord;
begin
  LSize := if FTextLength = 0 then GetPartialTextLength(AStart, AEnd) else FTextLength;
...
var
  ...
  LLineBreak: string;
begin
  LSize := if FTextLength = 0 then GetTextLength else FTextLength;
...
procedure TTextEditorLines.InvalidateTextCache;
begin
  FTextLength := 0;
end;
...
{$ENDIF}
  ...
  begin
    ...
    for var LLine := AIndex to AIndex + ACount - 1 do
...
procedure TTextEditorLines.SetLineBreakFlags(const ALineBreak: TTextEditorLineBreak);
begin
  for var LLine := 0 to Count - 1 do
  with Items^[LLine] do
  case ALineBreak of
    lbCRLF:
      begin
...
begin
  for var LIndex := AIndexFrom to AIndexTo do
  with FItems^[LIndex] do
  case ALineState of
    lsNormal:
...
begin
  for var LIndex := ABeginLine to AEndLine do
  with FItems^[LIndex] do
  case ATrimStyle of
    tsBoth:
      TextLine := FMX.TextEditor.Utils.Trim(TextLine);
    tsLeft:
      TextLine := FMX.TextEditor.Utils.TrimLeft(TextLine);
    tsRight:
      TextLine := FMX.TextEditor.Utils.TrimRight(TextLine);
  end;
end;
...
var
  LItem1: PTextEditorStringRecord;
  LItem2: PTextEditorStringRecord;
...
function TTextEditorLines.GetEncoding(const AStream: TStream; const AEncoding: System.SysUtils.TEncoding): System.SysUtils.TEncoding;
var
  LBufferSize: Int64;
  LBuffer: TBytes;
  LWithBOM: Boolean;
begin
  ...
  begin
    LBufferSize := if AStream.Size > FBufferSize then FBufferSize else AStream.Size;
    ...
    try
      if IsUTF8Buffer(LBuffer, LWithBOM) then
        Result := if LWithBOM then TEncoding.UTF8 else FMX.TextEditor.Encoding.TEncoding.UTF8WithoutBOM
      else
        System.SysUtils.TEncoding.GetBufferEncoding(LBuffer, Result);
    finally
      SetLength(LBuffer, 0);
    end;
  end;
...
procedure TTextEditorLines.LoadFromStream(const AStream: TStream; const AEncoding: System.SysUtils.TEncoding = nil);
var
  LBufferSize: Int64;
  LProgress: Int64;
...
begin
  ...
  Encoding := GetEncoding(AStream, AEncoding);
...
procedure TTextEditorLines.LoadFirstRows(const AStream: TStream; const AStringList: TStringList; const ARows: Integer; const AEncoding: System.SysUtils.TEncoding);
var
...
begin
  ...
  try
    ...
    LReader := TStreamReader.Create(AStream, AEncoding);
    try
      LCount := 0;

      while not LReader.EndOfStream and (LCount < ARows) do
      begin
...
procedure TTextEditorLines.LoadLastRows(const AStream: TStream; const AStringList: TStringList; const ARows: Integer; const AEncoding: System.SysUtils.TEncoding);
const
  CHUNK_SIZE = 4096;
var
  LSavedPosition: Int64;
...
begin
  ...
  try
    ...
    while (LFilePos > 0) and (LinesFound < ARows) do
    begin
      LChunkSize := Min(CHUNK_SIZE, LFilePos);
      LFilePos := LFilePos - LChunkSize;
      AStream.Position := LFilePos;
      LBytesRead := AStream.Read(LBuffer[0], LChunkSize);

      for var LIndex := LBytesRead - 1 downto 0 do
      if LBuffer[LIndex] = 10 then
      begin
        Inc(LinesFound);

        if LinesFound = ARows then
        begin
          LStartPos := LFilePos + LIndex + 1;
          Break;
        end;
      end;
    end;
    ...
    LReader := TStreamReader.Create(AStream, AEncoding, False);
    try
      while not LReader.EndOfStream do
        AStringList.Add(LReader.ReadLine);
    finally
      LReader.Free;
    end;
  finally
...
procedure TTextEditorLines.LoadPartFromStream(const AStream: TStream; const AFrom: TTextEditorPartialLoadFrom; const ARows: Integer; const AEncoding: System.SysUtils.TEncoding = nil);
var
  LStringList: TStringList;
...
begin
  ...
  try
    case AFrom of
      plfHead: LoadFirstRows(AStream, LStringList, ARows, Encoding);
      plfTail: LoadLastRows(AStream, LStringList, ARows, Encoding);
    end;

    LStringStream := TStringStream.Create;
    try
      LStringList.SaveToStream(LStringStream);

      LoadFromStream(LStringStream, Encoding);
    finally
      LStringStream.Free;
    end;
  finally
