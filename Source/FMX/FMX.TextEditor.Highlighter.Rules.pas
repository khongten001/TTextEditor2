...
unit FMX.TextEditor.Highlighter.Rules;

interface

uses
  System.Classes, System.SysUtils, FMX.TextEditor.Consts, FMX.TextEditor.Highlighter.Attributes, FMX.TextEditor.Highlighter.Token, FMX.TextEditor.Types;
...
type
  ...
  public
    ...
    procedure AddTokenRange(const AOpenToken: string; AOpenTokenBreakType: TTextEditorBreakType;
      const ACloseToken: string; const ACloseTokenBreakType: TTextEditorBreakType);
    procedure Clear;
...
uses
  System.UITypes, FMX.TextEditor.Utils;

{ TTextEditorParser }

constructor TTextEditorParser.Create(const AChar: Char; const AToken: TTextEditorToken;
...
procedure TTextEditorParser.AddTokenNode(const AString: string; const AToken: TTextEditorToken; const ABreakType: TTextEditorBreakType);
var
...
function TTextEditorParser.GetToken(const ACurrentRange: TTextEditorRange; const APLine: PChar; var ARun: Integer; var AToken: TTextEditorToken): Boolean;
var
  LStartPosition: Integer;
  LCurrentTokenNode: TTextEditorTokenNode;
  LNextPosition: Integer;
  LStartTokenNode: TTextEditorTokenNode;
  LFindTokenNode: TTextEditorTokenNode;
...
begin
  ...
  begin
    ...
    repeat
      ...
      LFindTokenNode := if Assigned(LCurrentTokenNode.Token) then LCurrentTokenNode else nil;
  ...
  for var LIndex := 0 to Sets.Count - 1 do
...
function TTextEditorDefaultParser.GetToken(const ACurrentRange: TTextEditorRange; const APLine: PChar; var ARun: Integer; var AToken: TTextEditorToken): Boolean;
begin
  Inc(ARun);
...
procedure TTextEditorRange.AddTokenRange(const AOpenToken: string; AOpenTokenBreakType: TTextEditorBreakType;
  const ACloseToken: string; const ACloseTokenBreakType: TTextEditorBreakType);
begin
...
begin
  ...
  for var LIndex := 0 to RangeCount - 1 do
...
begin
  ...
  begin
    for var LIndex := 0 to FRanges.Count - 1 do
    begin
      LRange := TTextEditorRange(FRanges[LIndex]);

      for var LIndex2 := 0 to LRange.FOpenToken.SymbolCount - 1 do
      begin
  ...
  begin
    for var LIndex := 0 to FKeyList.Count - 1 do
    begin
      LKeyList := TTextEditorKeyList(FKeyList[LIndex]);

      for var LIndex2 := 0 to LKeyList.KeyList.Count - 1 do
      begin
  ...
  begin
    for var LIndex := 0 to FTokens.Count - 1 do
    begin
      ...
      begin
        ...
        begin
          LToken := if LLength = 1 then LTempToken else FDefaultToken;
  ...
  begin
    for var LIndex := 0 to TCharacters.AnsiCharHigh do
    begin
      LAnsiChar := AnsiChar(CaseFunct(Char(LIndex)));

      for var LIndex2 := 0 to FSets.Count - 1 do
      begin
  ...
  begin
    ...
    if not Assigned(SymbolList[LAnsiChar]) then
      FSymbolList[LAnsiChar] := if LAnsiChar in FDelimiters then FDefaultTermSymbol else FDefaultSymbols;
  end;
...
begin
  ...
  begin
    ...
    if Assigned(SymbolList[LAnsiChar]) and (SymbolList[LAnsiChar] <> FDefaultTermSymbol) and
      (SymbolList[LAnsiChar] <> FDefaultSymbols) then
...
begin
  ...
  for var LIndex := FKeyList.Count - 1 downto 0 do
  begin
    LKeyList := TTextEditorKeyList(FKeyList[LIndex]);
...
begin
  ...
  FAttribute.Foreground := TAlphaColors.Black;
  FAttribute.Background := TAlphaColors.White;
end;
