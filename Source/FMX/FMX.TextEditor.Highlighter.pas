...
unit FMX.TextEditor.Highlighter;

interface

uses
  System.Classes, System.Generics.Collections, System.SysUtils, FMX.Controls, FMX.TextEditor.CodeFolding.Regions,
  FMX.TextEditor.Consts, FMX.TextEditor.Highlighter.Attributes, FMX.TextEditor.Highlighter.Colors,
  FMX.TextEditor.Highlighter.Comments, FMX.TextEditor.Highlighter.Rules, FMX.TextEditor.Highlighter.Token, FMX.TextEditor.Lines,
  FMX.TextEditor.SkipRegions, FMX.TextEditor.Types;
...
constructor TTextEditorHighlighter.Create(AOwner: TControl);
begin
  inherited Create;
...
begin
  ...
  for var LIndex := 0 to AKeywords.Count - 1 do
...
begin
  ...
  for var LIndex := 0 to ARange.SetCount - 1 do
    AddAttribute(ARange.Sets[LIndex].Attribute);

  for var LIndex := 0 to ARange.RangeCount - 1 do
...
begin
  for var LIndex := 0 to FTemporaryTokens.Count - 1 do
  begin
...
begin
  ...
  for var LIndex := 0 to FRange.AlternativeCloseArrayCount - 1 do
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        LDelimiters := if FRange.UseDelimitersForText then FRange.Delimiters + [TControlCharacters.Null] else FAllDelimiters;
...
begin
  Result := if Assigned(FToken) then FToken.Attribute else nil;
end;
...
begin
  ...
  for var LIndex := 0 to FMainRules.KeyListCount - 1 do
  begin
...
begin
  ...
  begin
    ...
    for var LIndex := 0 to FRange.KeyListCount - 1 do
    begin
...
begin
  ...
  for var LIndex := FMatchingPairs.Count - 1 downto 0 do
...
begin
  ...
  begin
    ...
    try
      ...
      for var LLine := FLines.Count - 1 downto 0 do
      begin
        ...
        begin
          ...
          begin
            ...
            if FMX.TextEditor.Utils.Trim(LKeyWord) <> ':' then
