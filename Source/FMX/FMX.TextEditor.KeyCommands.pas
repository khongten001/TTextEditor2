unit FMX.TextEditor.KeyCommands;
...
function TextEditorShortCut(const AKey: Word; const AShiftState: TShiftState): TShortCut;
...
begin
  ...
  for var LIndex := Low(EditorCommandStrings) to High(EditorCommandStrings) do
  begin
...
function TextEditorShortCutToText(const AShortCut: TShortCut): string;
var
  LKey: Word;
...
begin
  ...
  TextEditorShortCutToKey(AShortCut, LKey, LShiftState);
  Result := '';
...
begin
  Result := EditorCommandToCodeString(Command) + ' - ' + TextEditorShortCutToText(ShortCut);

  if SecondaryShortCut <> 0 then
    Result := Result + ' ' + TextEditorShortCutToText(SecondaryShortCut);
...
begin
  Result := TextEditorShortCut(Key, ShiftState);
end;
...
begin
  ...
  TextEditorShortCutToKey(AValue, LNewKey, LNewShiftState);
...
begin
  Result := TextEditorShortCut(SecondaryKey, SecondaryShiftState);
end;
...
begin
  ...
  begin
    ...
    for var LIndex := 0 to LKeyCommands.Count - 1 do
...
begin
  ...
  { Line operations }
  Add(TKeyCommands.InsertLine, [ssCtrl], vkReturn);
